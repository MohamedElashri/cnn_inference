...
Conv1d::Conv1d(int in_channels, int out_channels, 
    ...
    std::cout << "Conv1d Parameters:" << std::endl;
    ...
    try {
        ...
        // Set convolution descriptor for 1D conv as 2D conv
        ...
        // Set filter descriptor [out_channels, in_channels, 1, kernel_size]
        int filter_dims[] = {out_channels_, in_channels_, 1, kernel_size_};
        CUDNN_CHECK(cudnnSetFilterNdDescriptor(
            filter_desc_,
            CUDNN_DATA_FLOAT,
            CUDNN_TENSOR_NCHW,
            4,                      // 4D tensor for 2D conv
            filter_dims));
            
        // Allocate weights and bias
        weights_ = std::make_unique<Tensor<float>>(
            std::vector<int>{out_channels_, in_channels_, 1, kernel_size_});
...
Tensor<float> Conv1d::forward(const Tensor<float>& input, cudnnHandle_t cudnn_handle) {
    const auto& in_dims = input.dims();
    std::cout << "\nConv1d forward:" << std::endl;
    ...
    try {
        ...
        int seq_length = in_dims[2];
        int output_length = (seq_length + 2 * padding_ - kernel_size_) / stride_ + 1;

        // Set up tensor descriptors
        createTensor4dDesc(input_desc_, batch_size, in_channels_, 1, seq_length);
        ...
        std::cerr << "Error in Conv1d forward pass: " << e.what() << std::endl;
        throw;
    }
}

void Conv1d::loadWeights(const float* weights_data, const float* bias_data) {
